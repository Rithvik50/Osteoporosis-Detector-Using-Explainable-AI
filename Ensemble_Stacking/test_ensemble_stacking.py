...
class TestEdgeCases:
    ...
    def test_extra_features_raises_error(self, trained_model, train_test_data):
        """Test prediction with extra features raises appropriate error"""
        X_train, X_test, y_train, y_test = train_test_data
        ...
        # Should raise an error
        with pytest.raises(Exception):
            trained_model.predict(X_test_extra)
    ...
    def test_all_same_class(self):
        ...
        # This should fail during stratified split or handle gracefully
        # We expect some kind of exception here
        with pytest.raises(Exception):
            model = StackingEnsembleOptuna(cv_folds=2, random_state=42)
